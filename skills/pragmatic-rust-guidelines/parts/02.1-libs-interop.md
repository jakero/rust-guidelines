...
> Pragmatic Rust Guidelines - Part 02.1
> Source category: `libs/interop`

## Table of Contents

- **Types are Send (M-TYPES-SEND)**: use in Tokio and behind runtime abstractions.
- **Native escape hatches (M-ESCAPE-HATCHES)**: workarounds for unsupported use cases until alternatives exist.
- **Don't leak external types (M-DONT-LEAK-TYPES)**: stable APIs and low long-term maintenance cost.
- **Items come from their original crate (M-FOREIGN-REEXPORTS)**: unambiguous type identity.
- **Accept `impl AsRef<>` where feasible (M-IMPL-ASREF)**: flexibility for callers to use their own types.
- **Accept `impl RangeBounds<>` where feasible (M-IMPL-RANGEBOUNDS)**: flexibility and clarity when specifying ranges.
- **Accept `impl 'IO'` where feasible ('sans IO') (M-IMPL-IO)**: business logic untangled from I/O, with N*M composability.
...
> **Rationale**: use in Tokio and behind runtime abstractions.
...
> **Rationale**: workarounds for unsupported use cases until alternatives exist.
...
> **Rationale**: stable APIs and low long-term maintenance cost.
...
> **Rationale**: unambiguous type identity.
...
> **Rationale**: flexibility for callers to use their own types.
...
> **Rationale**: flexibility and clarity when specifying ranges.
...
> **Rationale**: business logic untangled from I/O, with N*M composability.
