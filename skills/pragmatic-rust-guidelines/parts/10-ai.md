...
- **Design with AI use in mind (M-DESIGN-FOR-AI)**: maximum utility from agents working in your codebase.
- **Items are only visible through one path (M-SINGLE-ITEM-PATH)**: a single, clutter-free path to each type.
- **Avoid meta design documentation (M-NO-META-DESIGN-DOCUMENTATION)**: docs focused on what is relevant to users.
- **Tests do not assert ground truth (M-TAUTOLOGICAL-TESTS)**: tests that add value, not noise.
- **Rust code solves Rust problems (M-RUST-SHAPED)**: idiomatic code.
...
> **Rationale**: maximum utility from agents working in your codebase.
...
along with the guidelines from [Library / UX](./02-2-libs-ux.md).
...
> **Rationale**: a single, clutter-free path to each type.
...
Similarly, re-exports of foreign items are not covered by this rule, although they should follow [M-FOREIGN-REEXPORTS](./02-1-libs-interop.md#M-FOREIGN-REEXPORTS).

Likewise, this rule also does not apply to public-but-hidden `_private` modules needed by macros, compare [M-MACRO-HELPERS](./03-macros.md#M-MACRO-HELPERS).
...
> **Rationale**: docs focused on what is relevant to users.
...
> **Rationale**: tests that add value, not noise.
...
While some language constructs simply don't translate at all (e.g., compared to C#, Rust does not have any meaningful reflection), others are deceptively similar and might only bite months down the line (e.g., statics, compare [M-AVOID-STATICS](./02-3-libs-resilience.md#M-AVOID-STATICS)).
