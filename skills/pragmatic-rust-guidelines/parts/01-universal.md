...
> Pragmatic Rust Guidelines - Part 01
> Source category: `universal`

## Table of Contents

- **Follow the upstream guidelines (M-UPSTREAM-GUIDELINES)**: a codebase that reflects community lessons and does not surprise users or contributors.
- **Use static verification (M-STATIC-VERIFICATION)**: consistency and freedom from common issues.
- **Lint overrides should use `#[expect]` (M-LINT-OVERRIDE-EXPECT)**: a current, tidy lint set.
- **Public types are Debug (M-PUBLIC-DEBUG)**: easy debugging without leaking sensitive data.
- **Public types meant to be read are Display (M-PUBLIC-DISPLAY)**: usability.
- **If in doubt, split the crate (M-SMALLER-CRATES)**: fast compile times and good modularity.
- **Names are free of weasel words (M-WEASEL-WORDS)**: readability.
- **Names of items are short (M-SHORT-NAMES)**: idiomatic code.
- **Prefer regular over associated functions (M-REGULAR-FN)**: readability.
- **Magic values are documented (M-DOCUMENTED-MAGIC)**: maintainability and safe refactoring.
- **Use structured logging with message templates (M-LOG-STRUCTURED)**: low-cost logging with strong filtering.
...
> **Rationale**: a codebase that reflects community lessons and does not surprise users or contributors.
...
> **Rationale**: consistency and freedom from common issues.
...
> **Rationale**: a current, tidy lint set.
...
> **Rationale**: easy debugging without leaking sensitive data.
...
> **Rationale**: fast compile times and good modularity.
...
Rust name is `Builder` (compare [M-INIT-BUILDER](./02-2-libs-ux.md#M-INIT-BUILDER)). A builder that can produce items repeatedly is still a builder.
...
around optional values (again, [M-INIT-BUILDER](./02-2-libs-ux.md#M-INIT-BUILDER)).
...
> **Rationale**: maintainability and safe refactoring.
...
> **Rationale**: low-cost logging with strong filtering.
