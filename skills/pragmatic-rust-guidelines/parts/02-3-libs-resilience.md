# Libraries - Resilience & Robustness

> Pragmatic Rust Guidelines - Part 02.3
> Source category: `libs/resilience`

## Table of Contents

- **I/O and system calls are mockable (M-MOCKABLE-SYSCALLS)**: testable edge cases that are otherwise hard to evoke.
- **Test utilities are feature gated (M-TEST-UTIL)**: production builds that cannot bypass safety checks.
- **Integration tests live under `tests/` (M-INTEGRATION-TESTS)**: clean code files.
- **Use the proper type family (M-STRONG-TYPES)**: the right data and safety invariants, at the right time.
- **Newtypes guard their invariants (M-STRONG-TYPES-GUARD)**: centralized correctness invariants.
- **Builders validate in final `.build()` (M-BUILD-RESULT)**: clean builder error handling.
- **Don't glob re-export items (M-NO-GLOB-REEXPORTS)**: a deliberate public surface.
- **Avoid statics (M-AVOID-STATICS)**: consistency and correctness across crate versions.
- **Production code uses telemetry, not println (M-LOG-NOT-PRINT)**: diagnostics available where they are needed.
...
> **Rationale**: testable edge cases that are otherwise hard to evoke.
...
> **Rationale**: production builds that cannot bypass safety checks.
...
> **Rationale**: the right data and safety invariants, at the right time.
...
> **Rationale**: centralized correctness invariants.
...
> **Rationale**: clean builder error handling.
...
> **Rationale**: a deliberate public surface.
...
> **Rationale**: consistency and correctness across crate versions.
...
> **Rationale**: diagnostics available where they are needed.
