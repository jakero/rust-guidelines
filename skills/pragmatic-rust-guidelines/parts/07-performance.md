...
> Pragmatic Rust Guidelines - Part 07
> Source category: `performance`

## Table of Contents

- **Optimize for throughput, avoid empty cycles (M-THROUGHPUT)**: COGS savings at scale.
- **Identify, profile, optimize the hot path early (M-HOTPATH)**: high-performance code.
- **Long-running tasks should have yield points (M-YIELD-POINTS)**: fair CPU time for all tasks.
- **Reuse allocations where possible (M-MEM-REUSE)**: low allocation overhead and fast hot paths.
- **Library telemetry does not tank performance (M-LOG-OVERHEAD)**: low-overhead telemetry during diagnosis.
- **Nested type hierarchies should avoid needless indirection (M-AVOID-INDIRECTION)**: fast, cache-friendly memory access.
- **Use boxed slices and strings for immutable owned sequences (M-BOX-DST)**: low memory consumption and good cache utilization.
- **Shrink collections to fit after building (M-SHRINK-TO-FIT)**: a minimal memory footprint.
- **Use a fast hasher where possible (M-FAST-HASHER)**: hashing performance.
- **Collections are created with sufficient initial capacity (M-INITIAL-CAPACITY)**: efficient collection creation.
- **Hot `async` functions reduce stack size (M-ASYNC-STACK-SIZE)**: small async stack sizes and low memcpy overhead.
...
> **Rationale**: COGS savings at scale.
...
> **Rationale**: high-performance code.
...
> **Rationale**: fair CPU time for all tasks.
...
> **Rationale**: low allocation overhead and fast hot paths.
...
> **Rationale**: low-overhead telemetry during diagnosis.
...
> **Rationale**: fast, cache-friendly memory access.
...
> **Rationale**: low memory consumption and good cache utilization.
...
> **Rationale**: a minimal memory footprint.
...
> **Rationale**: efficient collection creation.
...
> **Rationale**: small async stack sizes and low memcpy overhead.
