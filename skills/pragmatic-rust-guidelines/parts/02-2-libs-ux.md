...
> Pragmatic Rust Guidelines - Part 02.2
> Source category: `libs/ux`

## Table of Contents

- **Abstractions don't visibly nest (M-SIMPLE-ABSTRACTIONS)**: low cognitive load and a good out-of-the-box UX.
- **Avoid smart pointers and wrappers in APIs (M-AVOID-WRAPPERS)**: low cognitive load and ergonomic APIs.
- **Prefer types over generics, generics over dyn traits (M-DI-HIERARCHY)**: composable patterns and freedom from design lock-in.
- **Errors are canonical structs (M-ERRORS-CANONICAL-STRUCTS)**: harmonized error types and consistent error handling.
- **Canonical error conversion uses `From`, not `map_err` (M-FROM-ERROR)**: idiomatic error handling.
- **Complex type construction has builders (M-INIT-BUILDER)**: future-proof type construction in complex scenarios.
- **Complex type initialization hierarchies are cascaded (M-INIT-CASCADED)**: construction free of parameter mix-ups.
- **Services are Clone (M-SERVICES-CLONE)**: composable sharing of common services.
- **Essential functionality should be inherent (M-ESSENTIAL-FN-INHERENT)**: easily discoverable essential functionality.
- **Modules are balanced in size and scope (M-BALANCED-MODULES)**: discoverable functionality and clear API usage.
- **Don't define preludes (M-NO-PRELUDE)**: a clean namespace and reliable downstream builds.
- **Parameter ordering is consistent (M-PARAMETER-CONSISTENCY)**: low development friction.
- **Collections implement the appropriate iter traits (M-COLLECTION-TRAITS)**: composable collections.
- **Functions are `async` over returning a Future (M-ASYNC-FN)**: simpler code and easier-to-understand APIs.

---


<a id="M-SIMPLE-ABSTRACTIONS"></a>

## Abstractions don't visibly nest (M-SIMPLE-ABSTRACTIONS)

> **Rationale**: low cognitive load and a good out-of-the-box UX.
...
<a id="M-AVOID-WRAPPERS"></a>

## Avoid smart pointers and wrappers in APIs (M-AVOID-WRAPPERS)

> **Rationale**: low cognitive load and ergonomic APIs.
...
<a id="M-DI-HIERARCHY"></a>

## Prefer types over generics, generics over dyn traits (M-DI-HIERARCHY)

> **Rationale**: composable patterns and freedom from design lock-in.
...
<a id="M-ERRORS-CANONICAL-STRUCTS"></a>

## Errors are canonical structs (M-ERRORS-CANONICAL-STRUCTS)

> **Rationale**: harmonized error types and consistent error handling.
...
<a id="M-INIT-BUILDER"></a>

## Complex type construction has builders (M-INIT-BUILDER)

> **Rationale**: future-proof type construction in complex scenarios.
...
<a id="M-INIT-CASCADED"></a>

## Complex type initialization hierarchies are cascaded (M-INIT-CASCADED)

> **Rationale**: construction free of parameter mix-ups.
...
<a id="M-SERVICES-CLONE"></a>

## Services are Clone (M-SERVICES-CLONE)

> **Rationale**: composable sharing of common services.
...
<a id="M-ESSENTIAL-FN-INHERENT"></a>

## Essential functionality should be inherent (M-ESSENTIAL-FN-INHERENT)

> **Rationale**: easily discoverable essential functionality.
...
<a id="M-BALANCED-MODULES"></a>

## Modules are balanced in size and scope (M-BALANCED-MODULES)

> **Rationale**: discoverable functionality and clear API usage.
...
> **Rationale**: a clean namespace and reliable downstream builds.
...
<a id="M-PARAMETER-CONSISTENCY"></a>

## Parameter ordering is consistent (M-PARAMETER-CONSISTENCY)

> **Rationale**: low development friction.
...
> **Rationale**: simpler code and easier-to-understand APIs.

Functions should be declared `async fn foo()` over `fn foo() -> impl Future` when both are viable.

Functions marked `async` are more idiomatic and easier to read. An explicit `Future`-returning signature should only be used when required, for example inside traits or for _hot 'n heavy_ async functions, compare [M-ASYNC-STACK-SIZE](./07-performance.md#M-ASYNC-STACK-SIZE).
