...
- [`SKILL.md`](pragmatic-rust-guidelines/SKILL.md): 적용 지침, 작업별 빠른 색인(Quick Index), 분야별 라우팅 테이블 및 반영된 upstream 리비전 정보. 에이전트가 규칙을 찾는 시작점입니다.
...
지침 적용 전 프로젝트 요구사항, 라이브러리/애플리케이션 구분, 공개 API 여부, MSRV·런타임·배포 대상을 확인합니다. 링크는 스킬 디렉터리 기준으로 해석하며, 앵커 이동을 지원하지 않는 도구에서는 정확한 앵커 문자열을 검색한 뒤 다음 규칙 전까지 근거·예외·예제를 읽습니다. 범위 조회가 불가능하면 전체 파일을 읽어도 됩니다.

현재 원본에 없는 `M-RUNTIME-ABSTRACTED` 참조는 링크 없이 보존하고, 영향을 받는 규칙에 `Unavailable source reference` 안내를 생성합니다. 스킬만 복사한 에이전트도 해당 규칙의 내용을 추측하거나 다른 규칙으로 대체하지 않도록 안내합니다.
...
1. **upstream 소스 리비전 및 입력 무결성 확인**: 로컬 `upstream/main` 참조와의 공통 조상을 계산하여 `HEAD`의 `src/guidelines` 트리와 정확히 일치하는지, 작업 트리에 커밋되지 않은 수정이나 untracked/ignored 파일이 없는지 출력 파일 초기화 전에 검사합니다.
2. **원본 구조와 규칙 ID 확인**: 13개 분야 디렉터리와 `README.md`의 include 선언을 확인하고, 빠진 파일·중복 include·중복 규칙 ID 등 입력 오류를 생성 전에 검사합니다.
3. **기존 파트 초기화**: 출력 디렉터리를 준비하고 이전 빌드에서 생성한 `parts/*.md`를 지웁니다.
4. **분야별 파트 생성**: 원본 공개 순서와 분야별 include 순서로 13개 파트를 만듭니다. 각 파트에는 분야 소개, 규칙 목차와 근거, 정제한 본문을 담고, 이미지 태그와 단독 `<div>` 래퍼를 생략하며 내부 링크를 생성 파트로 바꿉니다. 원본에서 이름이 바뀐 과거 규칙 ID(`M-ABSTRACTIONS-DONT-NEST`, `M-DOC-FIRST-SENTENCE`)는 현재 정본 규칙 앵커로 정규화하며, 현재 원본에 대응 규칙 실체가 없는 미해결 참조(`M-RUNTIME-ABSTRACTED`)는 엉뚱한 위치로 연결되지 않도록 링크를 비활성화해 일반 텍스트 및 안내문으로 보존합니다. 동시에 `SKILL.md`의 라우팅 표에 쓸 분야·규칙 메타데이터를 모읍니다.
5. **파트와 앵커 확인**: 생성한 규칙 수가 원본 색인과 일치하는지, 모든 규칙 앵커가 해당 파트에 있는지 검사합니다.
6. **`SKILL.md` 생성**: `_build/SKILL.md.template`을 바탕으로 작업별 빠른 색인과 적용 지침, 에이전트 활용 가이드, 동적 라우팅 표 항목, 그리고 검증된 upstream 커밋 SHA와 커밋 날짜(ISO-8601)를 작성합니다.
7. **생성 링크 및 규칙 ID 검증**: 생성된 Markdown의 로컬 파일 링크와 규칙 앵커가 실제 대상에 연결되는지 전수 확인하고, `SKILL.md`에 언급된 모든 규칙 ID가 실제 유효한지 검증합니다.
8. **빌드 결과 출력**: 생성한 파트 수와 정제한 규칙 수, 출력 디렉터리 및 `SKILL.md` 경로를 표시합니다.
