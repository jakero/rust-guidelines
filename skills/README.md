...
5. **`SKILL.md` 생성**: `_build/SKILL.md.template`을 바탕으로 스킬 설명, 적용 지침, 에이전트 활용 가이드를 유지한 채 라우팅 표 항목을 동적으로 채워 `SKILL.md`를 작성합니다.
6. **생성 링크 검증**: 생성된 Markdown의 로컬 파일 링크와 규칙 앵커가 실제 대상에 연결되는지 확인합니다.
