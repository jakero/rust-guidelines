...
- [`SKILL.md`](pragmatic-rust-guidelines/SKILL.md): 적용 지침, 작업별 빠른 색인, 분야별 색인, 반영된 원본 저장소(upstream)의 리비전 정보. 에이전트의 시작점입니다.
...
`skills/_build/build_agent_skills.sh`를 실행하면 스크립트는 다음 8단계를 거쳐 스킬을 만듭니다.
...
3. **출력 디렉터리 초기화**: 대상 디렉터리를 준비하고 기존 `parts/*.md` 파일들을 모두 삭제합니다. 이후 파트 파일들을 처음부터 다시 작성하므로 생성 파트를 직접 수정한 내용은 재빌드 시 사라집니다.
4. **분야별 파트 작성**: 공식 공개 순서대로 13개 파트 파일을 작성합니다. 각 파일에는 분야 소개, 목차(TOC)와 규칙별 Rationale 요약, 그리고 정제된 규칙 본문이 순서대로 들어갑니다. 규칙 본문에는 고유 앵커 삽입, `<why>`의 `Rationale` 변환, 버전 태그 및 이미지 생략, `Tip: `·`Caution: ` 라벨 정규화, 파트 간 링크 재작성이 적용됩니다. 동시에 다음 단계에서 쓸 분야별 규칙 수와 ID 목록을 모읍니다.
5. **파트와 규칙 앵커 확인**: 실제 생성된 총 규칙 수가 2단계에서 파악한 원본 규칙 수와 정확히 일치하는지, 각 파트 파일에 해당 규칙의 고유 앵커(`<a id="M-..."></a>`)가 빠짐없이 들어갔는지 검사합니다.
6. **`SKILL.md` 작성**: `_build/SKILL.md.template`을 읽어 분야별 색인 표와 출처 리비전 정보를 실제 생성 데이터로 치환한 임시 파일을 만든 뒤 `SKILL.md`로 교체합니다. 작업별 빠른 색인과 핵심 적용 원칙은 템플릿의 내용이 유지됩니다.
7. **생성 문서의 연결 검사**: 생성된 모든 Markdown 파일 내의 상대 링크와 규칙 앵커 대상이 실제로 존재하는지 전수 검사합니다. 아울러 `SKILL.md`에 언급된 모든 규칙 ID가 2단계의 원본 규칙 목록에 존재하는 유효한 ID인지 검증합니다.
8. **완료 결과 출력**: 생성된 파트 수(13개), 정제된 규칙 수(89개), 스킬 디렉터리 및 진입점 파일 경로를 콘솔에 출력하고 종료합니다.
